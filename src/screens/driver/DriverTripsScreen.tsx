...
function CalendarView({ onDateSelect, initialDate, onMonthChange, reservations }: CalendarViewProps) {
  ...
  const reservationDates = useMemo(() => {
    return new Set(reservations.map(r => r.scheduled_at?.split('T')[0]));
  }, [reservations]);
      ...
      <View style={calStyles.grid}>
        {cells.map((day, idx) => {
          ...
          return (
            <TouchableOpacity
              key={`day-${day}`}
              style={[calStyles.cell, isSelected ? calStyles.cellSelected : (hasReservation && calStyles.cellHasReservation)]}
              onPress={() => handleDatePress(day)}
            >
              <Text style={[calStyles.cellText, isSelected && calStyles.cellTextSelected]}>{day}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
