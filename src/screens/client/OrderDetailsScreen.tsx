...
export default function OrderDetailsScreen() {
  ...
  const handleOpenPdf = async () => {
    if (!order?.pdf_url) {
      Alert.alert(
        'PDF non disponible',
        'Le document PDF pour cette course n\'est pas encore généré.',
      );
      return;
    }
    setOpeningPdf(true);
  ...
  const handleShare = async () => {
    ...
    try {
      const pdfUrl = order.pdf_url
        ? `${ordersApi.getPdfUrl(token, order.id)}?token=${encodeURIComponent(token)}`
        : null;
      await Share.share({
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.document}>
          ...
          <View style={styles.tripBlock}>
            ...
            {/* Passagers / bagages */}
            <View style={styles.tripMeta}>
              {snap.passengers_count != null && (
                <View style={styles.tripMetaItem}>
                  <Ionicons name="people-outline" size={14} color={Colors.textMuted} />
                  <Text style={styles.tripMetaText}>
                    {snap.passengers_count} passager{snap.passengers_count > 1 ? 's' : ''}
                  </Text>
                </View>
              )}
              {snap.luggage_count != null && (
                <View style={styles.tripMetaItem}>
                  <Ionicons name="briefcase-outline" size={14} color={Colors.textMuted} />
                  <Text style={styles.tripMetaText}>
                    {snap.luggage_count} bagage{snap.luggage_count > 1 ? 's' : ''}
                  </Text>
                </View>
              )}
            </View>
        ...
        {/* ── Bouton télécharger ── */}
        <TouchableOpacity
          style={[styles.pdfBtn, (!order.pdf_url || openingPdf) && styles.pdfBtnOff]}
