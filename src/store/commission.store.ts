...
interface CommissionSettingsState {
  ...
  fetchSummary:  (token: string, period: CommissionPeriod, date?: string) => Promise<void>;
  fetchCommissions: (token: string, filters: CommissionListFilters) => Promise<void>;
  clearError:    () => void;
}
...
export const useCommissionSettingsStore = create<CommissionSettingsState>((set) => ({
  ...
  fetchSummary: async (token, period, date) => {
    set({ isLoading: true, error: null });
    try {
      const res = await commissionApi.getSummary(token, period, date);
      if (!res.ok || !res.data) throw new Error(res.message ?? 'Erreur de chargement du résumé');
  ...
  fetchCommissions: async (token, filters) => {
    set({ isLoading: true, error: null });
    try {
      const res = await commissionApi.listCommissions(token, filters);
      if (!res.ok || !res.data) throw new Error(res.message ?? 'Erreur de chargement des commissions');
      set({
        commissions: res.data.commissions,
        commissionsTotal: res.data.total,
        commissionsPage: res.data.page,
        isLoading: false,
      });
    } catch (err: unknown) {
      set({ error: err instanceof Error ? err.message : 'Erreur inconnue', isLoading: false });
